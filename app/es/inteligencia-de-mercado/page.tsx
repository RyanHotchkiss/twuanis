...
import PestanasInteligenciaMercado from './PestanasInteligenciaMercado'
...
const engineFilters = {
  ...
  const hasFilters =
        ...
        await getPriceMeterAnalysis(engineFilters, 'es')

        const pricingStrategy =
        await getPricingStrategy(engineFilters, 'es')

        const marketScarcity =
        await getMarketScarcity(engineFilters, 'es')

        const marketMatches =
        await getMarketMatches(engineFilters, 'es')

        const valuation =
        await getValuation(engineFilters, 'es')

        const buyerDemand =
        await getBuyerDemand(engineFilters, 'es')
